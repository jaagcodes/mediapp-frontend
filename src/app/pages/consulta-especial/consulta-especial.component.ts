...
export class ConsultaEspecialComponent implements OnInit {
  ...
  //Nos sirve para saber cuando hemos escrito en el input, para así realizar el proceso de filtrado
  myControlPaciente: FormControl = new FormControl();
  myControlMedico: FormControl = new FormControl();

  //Observable: Resultado de los pacientes filtrados
  ...
  agregarExamen(){
    if(this.examenSeleccionado){
      ...
      for(let i = 0; i < this.examenesSeleccionados.length; i++){
        let examen = this.examenSeleccionado[i];
